...
func commands() -> void:
	var commands := []
	for command in console_commands:
		if console_commands[command].description == "" or (console_commands[command].description.begins_with(Globals.parse_text("console", "desc.cheat_tag")) and not Globals.cheats_enabled):
			continue
